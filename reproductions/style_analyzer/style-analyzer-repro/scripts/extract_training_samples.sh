...
EXTRACTION_AWK_SCRIPT="$SCRIPT_DIR/extract_training_samples.awk"

# Print out the CSV header
echo "repo, support, full_support"

# Find all test reports in the report directory, expecting them to be named according to the pattern:
#
#   PROJECT-train_report.md.
#
for report in $(find "$REPORT_DIR" -name '*-train_report.md')
do
    #echo report

    # Extract the project name from the path
    project_name="$(basename $report .md)"
    project_name="${project_name%%-train_report}"
    ...
    <"$report" awk -f $EXTRACTION_AWK_SCRIPT -v repository="${project_name}"
done